...
- **Handling out-of-vocabulary words** - A user input contains a rare technical term not seen during training
  - *Solution:* The tokenizer breaks the word into sub-word tokens (e.g., "unbelievable" → "un" + "believable"), allowing the model to process novel words by combining known sub-word components

- **Processing multilingual text** - Input contains mixed languages or non-English characters
  - *Solution:* Tokenizers are trained on diverse internet data with vocabulary covering multiple languages, punctuation, and character sequences, enabling cross-lingual token assignment

### Transformer Architecture Use Cases

- **Context-aware predictions** - Need to disambiguate words with multiple meanings (e.g., "bark" as dog sound vs. tree covering)
  - *Solution:* The attention mechanism weights surrounding tokens (e.g., "dog" increases weight for animal-related meaning, "tree" for plant-related), allowing context-appropriate embeddings and predictions

- **Efficient parallel processing** - Training on massive datasets requires computational efficiency
  - *Solution:* Multi-head attention evaluates multiple vector elements simultaneously, and positional encoding maintains sequence order without sequential processing bottlenecks

### Embedding Use Cases

- **Semantic search** - Finding documents conceptually related to a query, not just keyword matches
  - *Solution:* Calculate cosine similarity between query and document embeddings; vectors pointing in similar directions indicate semantic closeness regardless of exact word overlap

- **Analogy reasoning** - Solving "king is to queen as man is to ?"
  - *Solution:* Vector arithmetic on embeddings captures directional relationships in the multi-dimensional space, allowing the model to identify that the semantic shift from "man" parallels the shift from "king" to "queen"

### Decoder/Prediction Use Cases

- **Autoregressive text generation** - Generating coherent long-form content from a short prompt
  - *Solution:* Masked attention during training ensures the decoder only considers preceding tokens; during inference, iterative prediction appends each generated token back to the sequence for context-aware continuation

- **Preventing training data leakage** - Ensuring the model doesn't "cheat" by looking ahead at target tokens during training
  - *Solution:* Masked attention explicitly ignores tokens after the current position, forcing the model to learn genuine predictive patterns rather than memorizing target sequences
